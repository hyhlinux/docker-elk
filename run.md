...
#### log
```bash
docker run --log-driver=syslog --log-opt syslog-address=tcp://0.0.0.0:5000 --log-opt syslog-facility=daemon alpine ash
```


docker run --rm -it -p 5601:5601  \
    -v `pwd`/kibana/config/:/usr/share/kibana/config \
    dockerelk_kibana
##### logstash
```bash
input { stdin { } } output { stdout {  { elasticsearch { hosts => "elasticsearch:9200"} } }
...
docker run --rm -it  --link elk_e:elasticsearch  --link some-redis:redis -e LS_JAVA_OPTS="-Xmx256m -Xms256m" \
        -v `pwd`/logstash/config/logstash.yml:/usr/share/logstash/config/logstash.yml \
        -v `pwd`/logstash/pipeline:/usr/share/logstash/pipeline \
         dockerelk_logstash
